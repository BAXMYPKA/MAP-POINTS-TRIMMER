...
public class HtmlHandler {
	 ...
	 */
	public String processDescriptionText(String description, MultipartMainDto multipartMainDto, @Nullable LocalDateTime timeStampFromWhen) {
	 ...
	 */
	private void setPreviewSize(Element parsedHtmlFragment, MultipartMainDto multipartMainDto, @Nullable LocalDateTime timeStampWhen) {
...
//		Elements imgElements = parsedHtmlFragment.select("img[src]");
	...
	private void clearOutdatedDescriptions(
			Element parsedHtmlFragment, MultipartMainDto multipartMainDto, @Nullable LocalDateTime timeStampFromWhen) {
	 ...
	 *
	 * @param htmlElements      The Description as the HTML parsed markup.
	 * @param timeStampFromWhen {@link Nullable} The previously extracted timestamp from {@literal </Placemark></gx:TimeStamp></when>}.
	 *                          If present it will be used for comparison with the timestamp from the Description.
	 *                          And the earliest one will be inserted into the table row table data with the "Created" field.
	 ...
	 */
	private Elements getTableRowsWithMinDateTime(Elements htmlElements, @Nullable LocalDateTime timeStampFromWhen) {
		...
		Element tdElementWithMinimumDateTime = tdElementsWithDescription.stream()
				...
				.filter(e -> {
					...
					try {
						...
						log.trace("LocalDateTime from " + dateTimeFormatter + " has been parsed from td text content.");
						return true;
					} catch (DateTimeParseException ex) {
						try {
							LocalDateTime.parse(e.text(), DateTimeFormatter.ISO_OFFSET_DATE_TIME);
							log.trace("LocalDateTime from " + DateTimeFormatter.ISO_OFFSET_DATE_TIME + " has been parsed from td text content.");
							return true;
				...
				.min((e1, e2) -> {
					...
					try {
						dateTime1 = LocalDateTime.parse(e1.text(), dateTimeFormatter);
						dateTime2 = LocalDateTime.parse(e2.text(), dateTimeFormatter);
					} catch (DateTimeParseException e) {
						dateTime1 = LocalDateTime.parse(e1.text(), DateTimeFormatter.ISO_OFFSET_DATE_TIME);
						dateTime2 = LocalDateTime.parse(e2.text(), DateTimeFormatter.ISO_OFFSET_DATE_TIME);
					}
					return dateTime1.compareTo(dateTime2);
				})
				.map(element -> {
					if (timeStampFromWhen != null) {
