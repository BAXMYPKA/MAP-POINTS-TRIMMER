...
//		Element extendedData = document.createElementNS("http://www.locusmap.eu", "ExtendedData");
	 ...
	 * 2. Если есть родительский StyleMap - убиваем его. Если нет - убиваем сам Style <======= Not necessary
	...
	private void replaceInStyles(Element documentRoot, String pictogramName) {
		...
		if (stylesWithThumbnails.isEmpty()) return;
	 ...
	 */
	private void replacePlacemarksStyleUrlsWithNewStyle(String pictogramName, Map<String, Node> stylesWithThumbnails) {
		...
		kmlUtils.getStyleObjectsMap().values().stream()
			  ...
			  .ifPresent(existingStyleNode -> {
				  replacePlacemarksThumbnailStyleUrlsForStyles(stylesWithThumbnails, pictogramName);
		...
		if (!isReplacedWithExisting.get()) {
			...
			replacePlacemarksThumbnailStyleUrlsForStyleMaps(stylesWithThumbnails, existingStyleUrl);
		}
	}
	 ...
	 *
	 * @param stylesWithThumbnails {@literal Only <Style>s with the photo thumbnail 'id' attribute.
	 ...
	 */
	private void replacePlacemarksThumbnailStyleUrlsForStyles(Map<String, Node> stylesWithThumbnails, String newStyleUrl) {
		//<Placemark>s referring to <Style>s with photo thumbnails
		List<String> thumbnailsStyleUrlsFromPlacemarks = kmlUtils.getStyleUrlsFromPlacemarks();
		thumbnailsStyleUrlsFromPlacemarks.retainAll(stylesWithThumbnails.keySet());
		
		thumbnailsStyleUrlsFromPlacemarks.forEach(thumbnailStyleUrl -> {
			kmlUtils.getPlacemarksByStyleUrl(thumbnailStyleUrl)
     ...
     */
    private Node updateStyle(Node styleClone, String pictogramName) {
...
*/
	...
	private void replacePlacemarksStyleUrlsWithNewStyleMap(String pictogramName, Map<String, Node> styleMapsWithThumbnails) {
		...
		kmlUtils.getStyleObjectsMap().values().stream()
			  ...
			  .ifPresent(existingStyleMapNode -> {
				  replacePlacemarksThumbnailStyleUrlsForStyleMaps(styleMapsWithThumbnails, existingStyleUrl);
				  isReplacedWithExisting.set(true);
			  });
		if (!isReplacedWithExisting.get()) {
			//No previously created StyleMap with this pictogramName. Have to create a new one with the new <Style>s
	 ...
	 * @param newStyleUrl             {@literal It will replace all <Placemark>s <styleUrl>s.}
	 */
	private void replacePlacemarksThumbnailStyleUrlsForStyleMaps
	(Map<String, Node> styleMapsWithThumbnails, String newStyleUrl) {
		//<Placemark>s referring to <StyleMap>s with photo thumbnails
		List<String> thumbnailsStyleUrlsFromPlacemarks = kmlUtils.getStyleUrlsFromPlacemarks();
		thumbnailsStyleUrlsFromPlacemarks.retainAll(styleMapsWithThumbnails.keySet());
	 ...
	 */
	private Node cloneStyleObject(@NonNull Collection<Node> styleObjectssWithThumbnails) {
		Node styleObjectExample = styleObjectssWithThumbnails.iterator().next();
